                                               ...
                                               CommandsForKeyType extends SafeCommandsForKey> extends SafeCommandStore
{
    ...
    protected void update(Command prev, Command updated, @Nullable Seekables<?, ?> keysOrRanges)
    {
        ...
        Routables.foldl(keys, ranges, (self, p, key, u, i) -> {
            ...
            // TODO (required): we shouldn't special case invalidations: should ensure we have everything loaded, or permit asynchronous application
            Invariants.checkState(cfk != null || u.saveStatus().hasBeen(Status.Invalidated) || u.saveStatus().is(Status.AcceptedInvalidate));
            if (cfk != null)
                cfk.update(p, u);
            return u;
